    ...
    def draw(self):
        ...
        for week in weeks:
            for day in week['contributionDays']:
                ...
                # 绘制图像
                drawImage.line(
                    [(x_point, y_point), (x_point + square_width, y_point)], fill=color, width=square_width)
                # y 轴移动
                y_point += move_width
            # x 轴移动
            x_point += move_width
        ...
        percent_w, percent_h = drawImage.textsize(
            percent, font=big_title_font)  # 获取长宽
        drawImage.text((left_top_x - percent_w, 504), percent,
                       fill=big_title_color, font=big_title_font)

        precent_text1 = "你共有 %s 天提交代码" % total_day
        precent_text1_w, percent_text1_h = drawImage.textsize(
            precent_text1, font=text_font)
        drawImage.text((left_top_x - precent_text1_w, 627),
                       precent_text1, fill=text_color, font=text_font)
        precent_text2 = "占总天数的 %s" % percent
        precent_text2_w, precent_text2_h = drawImage.textsize(
            precent_text2, font=text_font)
        drawImage.text((left_top_x - precent_text2_w, 667),
                       precent_text2, fill=text_color, font=text_font)

        # 2. 连续天数 todo: 兼容长度
        right_top_x = 770
        max_continue_day = str(max_continue_day)
        drawImage.text((right_top_x, 551), max_continue_day,
                       fill=big_title_color, font=big_title_font)
        max_continue_day_text = "你共有 %s 天" % max_continue_day
        drawImage.text((right_top_x, 640), max_continue_day_text,
        ...
        if max_day_contribution_count == 0:
            ...
            max_day_contribution_date_list = max_day_contribution_date.split(
                "-")
            max_contribution_day_text1 = "%s 年 %s 月 %s 日" % (
                max_day_contribution_date_list[0], max_day_contribution_date_list[1], max_day_contribution_date_list[2])
            max_contribution_day_text2 = "你完成贡献 %s 次" % max_day_contribution_count

        max_day_contribution_count = str(max_day_contribution_count)
        max_contribution_w, max_contribution_h = drawImage.textsize(
            max_day_contribution_count, font=big_title_font)
        drawImage.text((left_bottom_x - max_contribution_w, 1598),
                       max_day_contribution_count, fill=big_title_color, font=big_title_font)

        max_contribution_day_text1_w, max_contribution_day_text1_h = drawImage.textsize(
            max_contribution_day_text1, font=text_font)
        drawImage.text((left_bottom_x - max_contribution_day_text1_w, 1490),
                       max_contribution_day_text1, fill=text_color, font=text_font)
        max_contribution_day_text2_w, max_contribution_day_text2_h = drawImage.textsize(
            max_contribution_day_text2, font=text_font)
        drawImage.text((left_bottom_x - max_contribution_day_text2_w, 1530),
                       max_contribution_day_text2, fill=text_color, font=text_font)
        # drawImage.text()
        ...
        drawImage.text((right_bottom_x, 1598), followers,
                       fill=big_title_color, font=big_title_font)
        follower_text = "%s 位追随者" % followers
        drawImage.text((right_bottom_x, 1543), follower_text,
                       fill=text_color, font=text_font)
        ...
        total_contributions_w, total_contributions_h = drawImage.textsize(
            total_contributions, font=total_font)
        drawImage.text((middle_x - total_contributions_w / 2, middle_y),
                       total_contributions, fill=text_color, font=total_font)
